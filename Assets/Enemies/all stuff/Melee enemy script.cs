...
public class Meleeenemyscript : Enemy
{
    ...
    bool playerinsight()
    {
        RaycastHit2D inshootingsight = Physics2D.BoxCast(boxCollider2.bounds.center +  range * transform.right * transform.localScale.x + transform.right, new Vector2(boxCollider2.bounds.size.x * size, boxCollider2.bounds.size.y), 0, Vector2.left, 0, playermask);
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(boxCollider2.bounds.center +  range *transform.right * transform.localScale.x + transform.right, new Vector3(boxCollider2.bounds.size.x * size, boxCollider2.bounds.size.y, boxCollider2.bounds.size.z));
    }
